...
namespace Assistant.Agents
{
  public class SellAgent : Agent
  {
    ...
    private void OnVendorSell(PacketReader pvSrc, PacketHandlerEventArgs args)
    {
      ...
      int delay = Config.GetInt("SellDelay");
      int sold = 0;
      ...
      if (list.Count > 0)
      {
        Client.Instance.SendToServer(new VendorSellResponse(vendor, list, delay));
    ...
    public override void OnSelected(ListBox subList, params Button[] buttons)
    {
      ...
      m_DelayButton.Text = Language.Format(LocString.SellDelay, Config.GetInt("SellDelay"));
    ...
    public override void OnButtonPress(int num)
    {
      switch (num)
      {
        ...
        case 6:
          ...
          break;
        case 7:
          InputDialogGump sellDelayGump = new InputDialogGump(OnChangeSellDelay, 0, Language.GetString(LocString.SellDelay), Config.GetInt("SellDelay").ToString());
          sellDelayGump.SendGump();
          break;
      }
    }
    ...
    private bool OnChangeSellDelay(int gfx, string amount)
    {
      if (int.TryParse(amount, out int parsedAmount))
      {
        Config.SetProperty("SellDelay", parsedAmount);
        m_DelayButton.Text = Language.Format(LocString.SellDelay, Config.GetInt("SellDelay"));
