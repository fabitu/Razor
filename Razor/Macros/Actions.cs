...
namespace Assistant.Macros
{
  ...
  public class TargetRelLocAction : MacroAction
  {
    ...
    public bool TryGetTile(Point3D prevPosition, out Ultima.HuedTile tile)
    {
      int x = prevPosition.X;
      int y = prevPosition.Y;
      int z = prevPosition.Z;
      try
      {
        tile = Map.GetTileNear(World.Player.Map, x, y, z);
        return true;
      }
      catch (Exception e)
      {
