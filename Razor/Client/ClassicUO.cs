...
namespace Assistant
{
  ...
  public class ClassicUOClient : Client
  {
    ...
    public override bool SendToServer(Packet p)
    {
      byte[] data = p.Compile();
      int length = (int)p.Length;
      return _sendToServer(ref data, ref length);
    }

    public override bool SendToServer(PacketReader pr)
    {
      return SendToServer(MakePacketFrom(pr));
    }

    public override bool SendToClient(Packet p)
    {
      byte[] data = p.Compile();
      int length = (int)p.Length;

      return _sendToClient(ref data, ref length);
    }

    public override bool SendPacketToClient(byte[] packet, int length)
    {
      var result = _sendToClient(ref packet, ref length);
      return result;
    }
