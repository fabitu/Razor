...
namespace Ultima
{
  ...
  public sealed class TileData
  {
    ...
    {
      filePath = "C:\\Program Files (x86)\\Ultima Online ElysiumUo\\tiledata.mul";
      filePath = Files.GetFilePath("tiledata.mul");
    ...
    public static List<ItemDataDto> GetItemDataDto()
    {
      List<ItemDataDto> result = new List<ItemDataDto>();
      using (StreamWriter Tex = new StreamWriter(new FileStream(filePath, FileMode.Create, FileAccess.ReadWrite),
        System.Text.Encoding.GetEncoding(1252)))
      {
        for (int i = 0; i < m_ItemData.Length; ++i)
        {
          ItemData tile = m_ItemData[i];
          var itemdata = new ItemDataDto()
          {
            ID = $"0x{i:X4}",
            Name = tile.Name,
            Background = (tile.Flags & TileFlag.Background) != 0,
            Weapon = (tile.Flags & TileFlag.Weapon) != 0,
            Transparent = (tile.Flags & TileFlag.Transparent) != 0,
            Translucent = (tile.Flags & TileFlag.Translucent) != 0,
            Wall = (tile.Flags & TileFlag.Wall) != 0,
            Damage = (tile.Flags & TileFlag.Damaging) != 0,
            Impassible = (tile.Flags & TileFlag.Impassable) != 0,
            Wet = (tile.Flags & TileFlag.Wet) != 0,            
            Surface = (tile.Flags & TileFlag.Surface) != 0,
            Bridge = (tile.Flags & TileFlag.Bridge) != 0,
            Generic = (tile.Flags & TileFlag.Generic) != 0,
            Window = (tile.Flags & TileFlag.Window) != 0,
            NoShoot = (tile.Flags & TileFlag.NoShoot) != 0,
            PrefixA = (tile.Flags & TileFlag.ArticleA) != 0,
            PrefixAn = (tile.Flags & TileFlag.ArticleAn) != 0,
            Internal = (tile.Flags & TileFlag.Internal) != 0,
            Foliage = (tile.Flags & TileFlag.Foliage) != 0,
            PartialHue = (tile.Flags & TileFlag.PartialHue) != 0,
            Map = (tile.Flags & TileFlag.Map) != 0,
            ContainerOrHeight = (tile.Flags & TileFlag.Container) != 0,
            Wearable = (tile.Flags & TileFlag.Wearable) != 0,
            Lightsource = (tile.Flags & TileFlag.LightSource) != 0,
            Animation = (tile.Flags & TileFlag.Animation) != 0,
            HoverOver = (tile.Flags & TileFlag.HoverOver) != 0,
            Armor = (tile.Flags & TileFlag.Armor) != 0,
            Roof = (tile.Flags & TileFlag.Roof) != 0,
            Door = (tile.Flags & TileFlag.Door) != 0,
            StairBack = (tile.Flags & TileFlag.StairBack) != 0,
            StairRight = (tile.Flags & TileFlag.StairRight) != 0,
          };
          result.Add(itemdata);
        }
      }
    ...
    public static List<LandDataDto> GetLandDataDto()
    {
      List<LandDataDto> result = new List<LandDataDto>();
      using (StreamWriter Tex = new StreamWriter(new FileStream(filePath, FileMode.Create, FileAccess.ReadWrite)))
      {   
        for (int i = 0; i < m_LandData.Length; ++i)
        {
          LandData tile = m_LandData[i];
          var landData = new LandDataDto()
          {
            ID = $"0x{i:X4}",
            Name = tile.Name,
            TextureID = $"0x{tile.TextureID:X4}",
            Background = ((tile.Flags & TileFlag.Background) != 0),
            Weapon = ((tile.Flags & TileFlag.Weapon) != 0),
            Transparent = ((tile.Flags & TileFlag.Transparent) != 0),
            Translucent = ((tile.Flags & TileFlag.Translucent) != 0),
            ...
            PartialHue = ((tile.Flags & TileFlag.PartialHue) != 0),
            Map = ((tile.Flags & TileFlag.Map) != 0),
            Container = ((tile.Flags & TileFlag.Container) != 0),
            Wearable = ((tile.Flags & TileFlag.Wearable) != 0),
            Lightsource = ((tile.Flags & TileFlag.LightSource) != 0),
            Animation = ((tile.Flags & TileFlag.Animation) != 0),
            HoverOver = ((tile.Flags & TileFlag.HoverOver) != 0),
  ...
  public class ItemDataDto
  {
    ...
    public string LayerOrQuality { get; set; }
    public string GumpOrAnimID { get; set; }
    public string Height { get; set; }
    public string Hue { get; set; }
    public string ClassOrQuantity { get; set; }
    public string StackingOffset { get; set; }
    ...
    public bool NoShoot { get; set; }
    public bool PrefixA { get; set; }
    public bool PrefixAn { get; set; }
    public bool Internal { get; set; }
    public bool Foliage { get; set; }
    public bool PartialHue { get; set; }
    public bool Unknown5 { get; set; }
    public bool Map { get; set; }
    public bool ContainerOrHeight { get; set; }
    ...
    public bool StairBack { get; set; }
    public bool StairRight { get; set; }
  }
